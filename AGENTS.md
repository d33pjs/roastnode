...
- Brew detail pages and hero cards should cross-link beans, grinder/machine equipment, and preparation tools where the target exists. Preparation tools currently link to anchors on the preparation tools index because they do not yet have detail pages. Use the shared `shared/back_link` partial for old "Back to..." links so mobile users get a real tap target.
...
- Bean detail analytics live in `BeanStatistics`; keep them scoped through the active workspace bean and query-backed until data volume justifies summaries. Bean list rows show primary photos and remaining amount as `remaining of bag size`; keep channeling on the bean detail page, not the bean list.
