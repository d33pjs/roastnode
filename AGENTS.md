...
- Preparation tools are method-scoped checklist records, not equipment. Brews snapshot selected preparation tool names and preselect active tools from the user's last brew. Preparation tools support detail/edit screens, additive photos, primary-photo/crop/remove media controls, manual position ordering, archive/reopen lifecycle, and query-backed detail analytics through `PreparationToolStatistics`.
