...
- Bean detail analytics live in `BeanStatistics`; keep them scoped through the active workspace bean and query-backed until data volume justifies summaries. Bean list rows may show compact per-bean summaries such as primary photo and channeling count/rate, but they must still use private media routes and workspace-scoped brew counts.
