...
- Branding assets live in `app/assets/images/brand/`. Use the shared brand partials for wordmarks and marks; the `*_transparent_bg.png` files currently contain baked checkerboards, so avoid them until they are replaced by true alpha-transparent exports.
