...
- Brew detail pages use a dense Hero Brew Card. Show `dose_grams` as Dose, show calculated brew ratio instead of duplicating beverage in the top metrics, render ratio time as smaller secondary text, render rating as visual bean marks, mark first drip in the chart when present, and display preparation tools from `BrewPreparationTool#tool_name` snapshots. Do not expose `email_address` on screenshot-friendly brew cards; use `User#display_label`, which falls back to `unknown username`.
