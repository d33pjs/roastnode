...
- Espresso brew logging requires an open bean. Default to the current user's last active brewed bean, then the first open bean. Redirect to bean creation when no open bean exists. Copy setup fields from the user's last brew, but keep subjective fields like rating, notes, channeling, and taste fresh.
