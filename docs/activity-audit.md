...
Anonymous views of ordinary public Brew, Bean, and Recipe shares and all public media reads stay only in share analytics. A Guest cupping page is the deliberate exception: its first successful access records one `brew.cupping_accessed` event and starts the feedback window. Failed authorization, failed sign-in, password-reset requests, other page visits, active-workspace switches, automatic Brew inventory rows, public snapshot refreshes, and backup-retention cleanup are not activity events.
