...
- Recipes were narrowed from the original broad v1 idea. The current implementation starts with private exact-target recipe profiles, guided logging overlays, and Hero Card ghost comparison; JSON portability, public recipe sharing, and machine-readable profile files remain separate follow-up slices.
