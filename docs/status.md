...
- Public brew sharing: workspace writers can publish their own brews through curated, optionally password-protected token pages with a snapshot-driven Hero Brew Card, selected non-cropped photos with a mobile-friendly fullscreen viewer, public notes, visible public links, prominent buy/affiliate links, bean public purchase/opening details when available, and refreshed snapshots after public-safe linked data changes. Owners/admins can manage any workspace share; viewers cannot. Public media is served only through the public share whitelist. Workspace settings show public share URLs, linked brews, edit/remove actions, total page views, and a capped recent full-IP view history.
