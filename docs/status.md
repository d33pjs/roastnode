...
- Beans: rich metadata, variety information, additive photos, primary photo, crop/download/remove media controls, derived bag statuses (`stock`, `open`, `finished`, transitional `used_up`, `archived`), normal finished-bag lifecycle with `finished_at` and used/days/grams-per-day stats, exceptional archive/discard, duplicate-as-new-bag, duplicate-label disambiguation, manual inventory adjustment UI, compact overview cards with rating and color-coded remaining bars, and danger-zone delete with dependent brew/inventory cleanup.
