...
- Recipe profiles: workspace-private espresso recipes can be created from existing brews, preserve public-safe source brew snapshots, expose editable exact targets, and show prominent target markers for grinder, dose, timing, yield, temperature, and watch notes. Recipe-guided logging keeps normal brew defaults unchanged, stores `recipe_id` plus a brew-time `recipe_snapshot`, and renders a subtle Hero Brew Card target ghost. Portable recipe JSON export/import creates unlinked private snapshots without auto-creating beans, equipment, tools, brews, or media. Viewers can read recipes but cannot create, edit, delete, export, import, or start recipe-guided logging.
...
- Recipes were narrowed from the original broad v1 idea. The current implementation starts with private exact-target recipe profiles, guided logging overlays, Hero Card ghost comparison, and JSON portability; public recipe sharing and machine-readable profile files remain separate follow-up slices.
