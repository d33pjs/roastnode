...
- Workspace settings: household name, currency, logo, banner, configurable Buy Me a Coffee support badge mode for public shared pages, and public brew-share management with URLs, linked brews, edit/remove actions, view counts, and retained recent viewer IP history.
...
- Beans: rich metadata, variety information, additive photos, primary photo, crop/download/remove media controls, derived bag statuses (`stock`, `open`, `finished`, transitional `used_up`, `archived`), grouped overview sections for open, stock, finished/used-up, and archived bags, active-bag sorting by newest brew use, normal finished-bag lifecycle with `finished_at` and used/days/grams-per-day stats, exceptional issue/archive card, duplicate-as-new-bag, duplicate-label disambiguation, manual inventory adjustment UI, compact overview/detail remaining cards with rating and color-coded remaining bars, and danger-zone delete with dependent brew/inventory cleanup.
...
- Presentation and setup polish: self-hosted Elms Sans, Roastnode brand assets, mobile-friendly back links, a global footer with a left-aligned GitHub badge plus right-aligned app version or configured Buy Me a Coffee support badge, cross-links between domain records, optional demo data, and documentation for each shipped slice.
