...
The public media controller streams only attachment IDs allowed by an enabled `PublicBrewShare` snapshot and the current live allowlist. New `public_media` manifests and legacy snapshot references are both intersected with live authorization before resolving an opaque handle. This includes selected brew/bean/equipment/tool photos, selected current Bean/Brew primaries used by the Hero, the workspace logo, the logger avatar, and a household-recipient avatar only while that User remains a current member of the Brew workspace. Membership is authorized before the recipient attachment is read; removal or avatar replacement invalidates stale handles, and a replacement is unavailable until snapshot refresh. Unsupported variants, disabled shares, unknown tokens, locked password-protected shares, and attachments outside the public whitelist return `404 Not Found`.
...
The public bean media controller intersects the snapshot's recursively derived `public_media` manifest with a live allowlist before resolving an opaque handle. The live allowlist contains selected bean package photos, the workspace logo, logger avatars, and household-recipient avatars only for current workspace members. Recipient membership is checked before recipient attachment/blob records are loaded. Guest, unknown, malformed, and former-member recipient projections cannot render a recipient avatar, even when the same attachment is legitimately allowlisted elsewhere in the Bean snapshot.

The complete live authorization result is memoized only on the loaded `PublicBeanShare` instance and cleared on save or reload, bounding repeated card URL generation without creating a cross-request cache. A fresh request therefore rejects a removed member's or replaced avatar's stale handle immediately; a replacement avatar remains unavailable until a refreshed snapshot includes it. Disabled shares, unknown tokens, locked password-protected shares, unsupported variants, deleted attachments, unselected bean photos, and attachments outside either allowlist return `404 Not Found`.
