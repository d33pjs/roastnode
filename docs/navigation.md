...
- Bean Analytics Recent brews links to `/coffees` with Brews active and a removable active-Bean chip. The Bean selection survives Compact/Hero, compatible Coffee-type filters, and pagination. External is incompatible and clears it; unknown/nonexistent, non-scalar, or foreign-workspace Bean IDs return not found, while an omitted or blank filter shows normal unfiltered Coffees.
- Activity history is available at `/activity` and uses the same dashboard-style activity cards. It filters by category, actor, inclusive start date, and inclusive end date; pagination preserves all filters. Category-colored SVG icon containers and explicit Quick Drip wording distinguish Espresso, Quick Drip, and External Coffee; [Activity audit](activity-audit.md) defines the durable ledger and visibility contract.
...
- Gear is a top-level destination and its back button always returns to the dashboard. Maintenance create/edit actions remain previous-screen-aware, falling back to their explicit parent when there is no safe same-origin referrer: Gear for new logs and the event detail for edits. A failed maintenance save uses that fallback instead of treating the form itself as a useful previous screen. Saved maintenance detail pages always return to the dashboard, so the create/edit form cannot reappear in a back-link loop after a redirect.
...
- Back links use the shared `shared/back_link` partial as icon-only left-arrow buttons with `aria-label` and `title` text for accessibility and desktop hover. Action screens can use a safe same-origin previous page and otherwise fall back to the explicit path passed by the view. Direct visits, refreshes, self-referrers, external referrers, and share workflow referrers use the fallback so workflows cannot loop. Top-level Gear, brew log creation, and brew detail pages opt out of previous-page behavior and always target the dashboard.
