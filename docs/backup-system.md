...
Coffee history remains compatible with archive format/version `1`. New workspace payloads include `coffee_histories` and every Bean's `coffee_history_id`; both are authoritative together. Validation rejects duplicate/malformed group IDs and missing or foreign memberships, and restore remaps them within each workspace. Older archives lacking both fields reconstruct groups from valid restored duplicate ancestry; partially supplied fields fail closed. Explicitly separate groups and surviving membership after source-bag deletion round-trip unchanged. History groups also participate in the empty-target check and workspace cleanup. See [Grinder history](grinder-history.md).
