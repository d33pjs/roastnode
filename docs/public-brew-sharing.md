...
The public Hero Brew Card mirrors the private card's brew curve, timing guides, first-drip callout, total-time marker, and vertical temperature callout, but it does not render recipe ghost targets or the private retention card. Its upper section uses the shared two-photo backdrop: a selected Bean primary is contained on the left, a selected Brew primary covers the right, and unselected or unavailable halves stay black. A snapshot-only byline identifies Self, a household member, a generic Guest, or an unknown recipient truthfully. Small logger and currently authorized household-recipient avatars may accompany that byline through opaque public-media handles; the existing household/user identity strip remains below the Hero.
...
- selected current Bean/Brew primaries referenced by the Hero snapshot
