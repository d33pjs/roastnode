...
Workspace name/logo and logger display labels/avatars are intentional public identity surfaces when copied into the snapshot media allowlist. Recipient projection is automatic for every Brew summary; there is no per-share recipient setting. Self is kind-only. Named and unnamed Guests are both kind-only and render as **a guest**, never with the private Guest name or Cup style. A household recipient uses only the safe `User#display_label`; a former member keeps that safe snapshotted label but has no recipient avatar, while a current member may have one only after live Workspace membership is authorized before attachment access. Malformed recipient data falls back to **someone** without a borrowed avatar. Identity images must still render through public media handles, not raw Active Storage URLs.
