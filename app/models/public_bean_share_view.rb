class PublicBeanShareView < ApplicationRecord
  ...
  private
    def set_workspace_from_share
      self.workspace ||= public_bean_share&.workspace
    end
    ...
    def workspace_matches_share
      return if workspace.blank? || public_bean_share.blank?
      return if workspace_id == public_bean_share.workspace_id

      errors.add(:workspace, "must match the public bean share workspace")
    end

    def prune_old_rows
      old_rows = public_bean_share.public_bean_share_views.recent.offset(RETAINED_ROWS_PER_SHARE).select(:id)
      public_bean_share.public_bean_share_views.where(id: old_rows).delete_all
    end
end
