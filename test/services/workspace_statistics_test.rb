...
class WorkspaceStatisticsTest < ActiveSupport::TestCase
  test "builds workspace scoped coffee statistics" do
    ...
    today = Date.current
    assert_equal 2, statistics[:series][:brews_by_day].detect { |point| point[:date] == today }[:count]
    assert_equal 38.to_d, statistics[:series][:consumption_by_day].detect { |point| point[:date] == today }[:grams]
